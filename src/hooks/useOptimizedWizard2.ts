...
const STORAGE_KEY = "chatbot-wizard-draft-v3"
...
const INITIAL_DATA: ChatbotWizardData = {
  ...
  // ⬇️ Add this block to satisfy the interface
  getUserInfo: {
...
console.log("INITIAL_DATA role:", INITIAL_DATA.role);
...
export function useOptimizedWizard() {
  ...
  const validateBasicInfo = useCallback((data: ChatbotWizardData): ValidationResult => {
    ...
    // Always allow clicking Next button - validation will be handled in the component
    // if (!botname) {
    ...
    return { isValid: true, errors } // Always return true to keep Next button enabled
  }, [])
  ...
  const validateGetUserInfoSetup = useCallback((data: ChatbotWizardData): ValidationResult => {
    const errors: string[] = []
    const { message, initialPrompts } = data.welcome
  ...
  const validatePersonalInfo = useCallback((data: ChatbotWizardData): ValidationResult => {
    const errors: string[] = []
  ...
  const validateStep = useCallback(
    (stepIndex: number, dataToValidate: ChatbotWizardData): ValidationResult => {
      switch (stepIndex) {
        ...
        case 3:
          return validateGetUserInfoSetup(dataToValidate)
        case 4:
          return validatePersonalInfo(dataToValidate)
        default:
          return { isValid: true, errors: [] }
      }
    },
    [validateBasicInfo, validateKnowledgeSources, validateAppearance, validateWelcomeSetup, validateIntegration, validatePersonalInfo],
  )
