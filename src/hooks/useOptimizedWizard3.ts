...
const INITIAL_DATA: ChatbotWizardData = {
  ...
  appearance: {
    avatar: "/assets/img/brief/avatar1.png",
    primaryColor: "#3b82f6",
...
export function useOptimizedWizard3() {
  ...
  useEffect(() => {
    const loadDraft = async () => {
      try {
        ...
        const oldKeys = ["chatbot-wizard-draft", "chatbot-wizard-draft-v2"];
  ...
  const validateCustomization = useCallback(
    (data: ChatbotWizardData): ValidationResult => {
      const errors: string[] = [];
  ...
  const validateBasicSetup = useCallback(
    (data: ChatbotWizardData): ValidationResult => {
      const errors: string[] = [];
      return { isValid: true, errors };
    },
  ...
  const validateUseCase = useCallback(
    (data: ChatbotWizardData): ValidationResult => {
      const errors: string[] = [];
      return { isValid: true, errors };
    },
  ...
  const validateTrainingData = useCallback(
    (data: ChatbotWizardData): ValidationResult => {
      const errors: string[] = [];

      // Check if either URL or files are provided
      const hasUrl =
        data.knowledgeSources?.urls &&
        data.knowledgeSources.urls.length > 0 &&
        data.knowledgeSources.urls[0]?.trim().length > 0;
      const hasFiles =
        data.knowledgeSources?.files && data.knowledgeSources.files.length > 0;

      // if (!hasUrl && !hasFiles) {
      //   errors.push(
      //     "Please provide either a website URL or upload at least one file to continue"
      //   );
      // }
  ...
  const validateAboutYou = useCallback(
    (data: ChatbotWizardData): ValidationResult => {
      const errors: string[] = [];
  ...
  const validateStep = useCallback(
    (
      stepIndex: number,
      dataToValidate: ChatbotWizardData
    ): ValidationResult => {
      switch (stepIndex) {
        case 0:
  ...
  useEffect(() => {
    setSteps((prev) =>
      prev.map((step, index) => ({
        ...
        isComplete:
          index < currentStep ||
          (index === currentStep && stepValidations[index].isValid),
      }))
    );
