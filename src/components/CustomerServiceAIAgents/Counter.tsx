...
const Counter: React.FC<CounterProps> = ({ data, labels, units, title, subtitle }) => {
  ...
  const animateCounter = (): void => {
    ...
    const timer: NodeJS.Timeout = setInterval(() => {
      ...
      const newCounts: CounterState = {
        ...
        roi: parseFloat((data.roi * progress).toFixed(1))
      };
      ...
      const observer: IntersectionObserver = new IntersectionObserver(
        (entries: IntersectionObserverEntry[]) => {
          entries.forEach((entry: IntersectionObserverEntry) => {
            console.log('Counter intersection:', entry.isIntersecting, hasAnimated);
