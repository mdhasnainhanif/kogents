...
void main() {
  ...
  useEffect(() => {
    ...
    // Apply mobile-specific adjustments here in JS, not in the shader
  ...
  // Don't render LightRays on mobile devices
  if (isMobile) {
    return null;
  }
  ...
  return (
    <div
      ref={containerRef}
      className={`w-full h-full pointer-events-none z-[3] overflow-hidden lightRays${className}`.trim()}
    />
  );
};
