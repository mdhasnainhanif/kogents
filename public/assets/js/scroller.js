...
(function() {
  ...
  function initStackCardsResize(element) {
    element.element.addEventListener('resize-stack-cards', function(){
      element.isDesktop = window.innerWidth >= 1024;
      setStackCards(element);
  ...
  function stackCardsScrolling() {
    if (!this.isDesktop || this.scrolling) return;
    this.scrolling = true;
  ...
  function animateStackCards() {
    if (!this.isDesktop || isNaN(this.marginY)) {
      this.scrolling = false;
      return;
    }
  ...
  // Initialize only if IntersectionObserver is supported and not reduced motion
